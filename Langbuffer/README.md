...
This is the only current version of Langbuffer. It uses local speech recognition, local translation, and delayed audiovisual playback to prepare subtitles before the viewer hears the corresponding speech.

## Start

Double-click **Start Langbuffer.cmd**. The portable release includes Python, Qt, the `small.en` recognition model, and English-to-Spanish OPUS translation. You do not need to install Python or use Windows Live Captions. This development installation may contain additional models and language packages. The original Spanish-named launcher remains available.
...
| `src/langbuffer/` | Application source code |
...
The YouTube and Facebook resolvers run in cancelable child processes. YouTube uses the bundled Node.js and `yt-dlp-ejs`; no system Node.js installation is required. Temporary media URLs are not written to metrics. A short real YouTube sample was checked; Facebook media playback was checked synthetically.
...
```powershell
.\runtime\python.exe -B -m langbuffer
...
Dependencies are pinned in `requirements-lock.txt`. For source development, use Python 3.12 x64, install that lock file, and add `src/` to `PYTHONPATH` before running `python -m langbuffer`. The portable runtime is a local assembled distribution, not a Windows installer.
