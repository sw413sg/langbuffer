...
- **Systran speech models:** `small.en` is bundled; `base.en` and multilingual `small` can be downloaded. The bundled model converts [OpenAI Whisper small.en](https://huggingface.co/Systran/faster-whisper-small.en), listed as MIT; the [original MIT notice](third_party/whisper-model-LICENSE) is included. Repositories, revisions, and integrity checks are in `src/langbuffer/model_catalog.py`, `src/langbuffer/package_catalog.json`, and download manifests.
