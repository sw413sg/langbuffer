"""Live Translate: local recognition, translation and delayed playback."""
import argparse
import json
import math
import os
...
from live_translate.pipeline import ROOT, WORK, ASR_MODELS, model_path
...
from live_translate.settings import SettingsStore, DEFAULTS, validated
from live_translate.media_source import content_url, content_kind
from live_translate.playback import LocalPlayback
from live_translate.language_packages import LANGUAGES
from live_translate.language_controls import LanguageControls
from live_translate.ui_localization import UiLocalization
...
def validate_local_start(panel):
    try:
        ...
        if content_kind(url) not in {'broadcast', 'twitch_live', 'kick_live', 'youtube_live', 'facebook'}:
            raise ValueError('Live Translate supports X broadcasts and Twitch/Kick channels.')
...
def main():
    ...
    app = QApplication([])
    from live_translate.instance_lock import acquire_instance
    legacy_lock = acquire_instance(WORK)
    if legacy_lock is None:
        from PySide6.QtWidgets import QMessageBox
        language = AppSettings().load()[0].get('output_language', 'en')
        message = {
            'en': 'Close the older Live Translate window once before opening multiple windows with this update. If none is open, check data folder permissions.',
            'es': 'Cierra una vez la ventana anterior de Live Translate antes de abrir varias con esta actualización. Si no hay ninguna abierta, revisa los permisos de la carpeta de datos.',
            'pt': 'Feche uma vez a janela anterior do Live Translate antes de abrir várias com esta atualização. Se nenhuma estiver aberta, verifique as permissões da pasta de dados.',
            'fr': 'Fermez une fois l’ancienne fenêtre de Live Translate avant d’en ouvrir plusieurs avec cette mise à jour. Si aucune n’est ouverte, vérifiez les droits du dossier de données.',
        }
        QMessageBox.information(None, 'Live Translate', message.get(language, message['en']))
        return 1
    legacy_lock.unlock()
