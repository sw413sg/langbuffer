...
class MediaClockTests(unittest.TestCase):
    def test_multiple_pauses_excluded_without_growing_history(self):
        clock = MediaClock()
        with patch('live_translate.media_clock.time.perf_counter', return_value=100):
            clock.pause()
        with patch('live_translate.media_clock.time.perf_counter', return_value=130):
            self.assertEqual(clock.now(), 100)
            clock.pause()
            clock.resume()
        with patch('live_translate.media_clock.time.perf_counter', return_value=140):
            self.assertEqual(clock.now(), 110)
            clock.pause()
        with patch('live_translate.media_clock.time.perf_counter', return_value=150):
            clock.resume()
            self.assertEqual(clock.now(), 110)
