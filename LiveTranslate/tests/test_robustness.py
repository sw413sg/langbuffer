...
import numpy as np
from live_translate import pipeline, playback, language_packages as packages, seekable_hls, transport
...
class WorkerChecks(unittest.TestCase):
    def run_worker(self, asr, translator, count=2):
        ...
        with patch('faster_whisper.WhisperModel', return_value=asr), \
                patch('live_translate.local_translation.LocalTranslation', return_value=translator), \
...
class LifecycleChecks(unittest.TestCase):
    ...
    def test_legacy_instance_lock_is_released_and_recovers_after_crash(self):
        with tempfile.TemporaryDirectory(dir=packages.WORK, prefix='lock-check-') as temporary:
            ...
            code = ('from live_translate.instance_lock import acquire_instance; import sys; '
            ...
            crash = ('from live_translate.instance_lock import acquire_instance; import os,sys; '
